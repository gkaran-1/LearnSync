            ...
            return (
              <Link
                ...
                className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
                  ${isActive
                    ? isAITutor
                      ? 'bg-gradient-to-r from-blue-600 to-sky-600 text-white shadow-md shadow-blue-200'
                      : 'bg-blue-600 text-white shadow-md shadow-blue-200'
                    : isAITutor
                      ? 'bg-gradient-to-r from-blue-50 to-sky-50 text-blue-700 border border-blue-200 hover:from-blue-100 hover:to-sky-100'
                      : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                  }`}
              >
                <item.icon className={`w-4.5 h-4.5 flex-shrink-0 ${isActive ? 'text-white' : isAITutor ? 'text-blue-500' : 'text-slate-400 group-hover:text-slate-700'}`} style={{ width: 18, height: 18 }} />
                <span className="flex-1">{item.name}</span>
                {isActive && <ChevronRight className="w-3.5 h-3.5 text-white/70" />}
                {isAITutor && !isActive && <span className="w-2 h-2 rounded-full bg-blue-400 pulse-dot" />}
              </Link>
            );
          })}
        </nav>
